...
First, install both the `docsify` and `docsify-material` package for Python: 

```{bash}
pip install docsify docsify-material
```
...
Edit the `altdoc/docsify.html` file. What follows is a minimal example in which took the default Docsify settings that `altdoc` creates, and we add only two lines of code.

Instead of the default Docsify theme, we insert:

```html
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/docsify-themeable@0/dist/css/theme-simple.css">
```

After the Docsify script, we insert:
