.import_settings_mkdocs <- function(path) {
    ...
    # render mkdocs
    if (.is_windows()) {
        ### TODO: (cd <path> && python -m mkdocs build -q) should automatically go back to the previous directory
        ...
        cmd <- paste("cd", fs::path_abs(path), "&& python -m mkdocs build -q")
        shell(cmd)
        shell(paste("cd", goback))
    } else {
        goback <- getwd()
        cmd <- paste(fs::path_abs(path), "&& python -m mkdocs build -q")
        system2("cd", cmd)
        system2("cd", goback)
    }
